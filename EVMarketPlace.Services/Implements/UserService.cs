...
namespace EVMarketPlace.Services.Implements
{
    public class UserService : IUserService
    {
        ...
        public async Task<BaseRespone> ChangePasswordAsync(ChangePasswordRequest request)
        {
            ...
            if (!isValidOtp)
            {
                throw new NotFoundException("OTP không h?p l? ho?c ð? h?t h?n.");
            ...
            return new BaseRespone
            {
                ...
                Message = "Ð?t l?i m?t kh?u thành công."
            };
        }
        ...
        public async Task<BaseRespone> CreateAccount(CreateAccountRequest request)
        {
            ...
            if (existingUser != null)
            {
                ...
                throw new NotFoundException("Email ð? ðý?c ðãng k?!");
            }
            var user = new User
            {
                UserId =  Guid.NewGuid(),
            ...
            await _userRepository.CreateAsync(user);// lýu vào DB
        ...
        public async Task<BaseRespone> LoginAsync(LoginRequest request)
        {
            ...
            if (account == null)
            {
                return new BaseRespone
                {
                    ...
                    Message = "Ðãng nh?p th?t b?i. Vui l?ng ki?m tra l?i email và m?t kh?u.",
                    Data = null
                };
            }
            ...
            // Cho phép ðãng nh?p ngay c? khi chýa kích ho?t
            // Nhýng tr? v? thông tin ð?c bi?t ð? frontend x? l?
            return new BaseRespone
            {
                Status = StatusCodes.Status200OK.ToString(),
        ...
        public async Task<BaseRespone> VerifyOtpActiveAccountAsync(string email, string opt)
        {
            ...
            {
                Status = StatusCodes.Status200OK.ToString(),
                Message = "Xác minh OTP thành công. Tài kho?n ð? ðý?c kích ho?t.",
                Data = new
                {
                    user.FullName,
                    user.Email,
        ...
        public async Task<BaseRespone> ForgotPasswordAsync(string email)
        {
            ...
            var html = $"<p>M? OTP ð? ð?t l?i m?t kh?u: <b>{otp}</b></p><p>M? s? h?t h?n sau 5 phút.</p>";
            await _emailSender.SendEmailAsync(email, "Ð?t l?i m?t kh?u", html);

            return new BaseRespone
            {
                Status = StatusCodes.Status200OK.ToString(),
                Message = "M? OTP ð? ðý?c g?i ð?n email c?a b?n."
            };
        }
        ...
        public async Task<BaseRespone> ResendOtpAsync(string email)
        {
            ...
            // Ki?m tra user ð? active chýa
            if (user.IsActive)
            {
                return new BaseRespone
                {
                    Status = StatusCodes.Status400BadRequest.ToString(),
                    Message = "Tài kho?n ð? ðý?c kích ho?t."
                };
            }

            // T?o và g?i OTP m?i
            var otp = _otpService.GenerateAndSaveOtp(email, 5);
            var html = $"<p>M? OTP ð? kích ho?t tài kho?n: <b>{otp}</b></p><p>M? s? h?t h?n sau 5 phút.</p>";
